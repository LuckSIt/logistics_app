...
# -*- coding: utf-8 -*-
"""
Специализированный парсер для авиационных тарифов
"""
...
class AirParser(BaseParser):
    ...
    def __init__(self):
        super().__init__()
        self.analyzer = AirAnalyzer()
        self.transport_type = "air"
        
    def parse_tariff_data(self, file_path: str, supplier_id: int) -> List[Dict[str, Any]]:
        """
        Парсинг авиационных тарифов из файла.
        ...
        """
        logger.info(f"Начинаем парсинг авиационных тарифов из файла: {file_path}")
        ...
        parsed_data = self.analyzer.extract_air_routes(text)
        ...
        logger.info(f"Извлечено {len(tariffs)} авиационных тарифов")
        return tariffs
    ...
    def _convert_to_standard_format(self, data: Dict[str, Any], supplier_id: int, source_file: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Определяем базис (по умолчанию EXW для авиа)
            basis = data.get('basis', 'EXW')
            
            # Извлекаем тип груза/услуги
            vehicle_type = data.get('vehicle_type', '')
            if not vehicle_type:
                # Пытаемся определить по контексту
                if any(keyword in data.get('description', '').lower() for keyword in ['express', 'экспресс']):
                    vehicle_type = 'express'
                elif any(keyword in data.get('description', '').lower() for keyword in ['standard', 'стандарт']):
            ...
            logger.error(f"Ошибка преобразования данных авиационного тарифа: {e}")
            return None
    ...
    def detect_air_keywords(self, text: str) -> bool:
        """
        Определяет, содержит ли текст ключевые слова авиационного транспорта.
        
        Args:
            text: Текст для анализа
            
        Returns:
            True если текст содержит авиационные ключевые слова
        """
        air_keywords = [
            'авиа', 'air', 'flight', 'рейс', 'самолет', 'airplane', 'aircraft',
            'аэропорт', 'airport', 'terminal', 'терминал', 'gate', 'гейт',
            'express', 'экспресс', 'cargo', 'груз', 'freight', 'фрахт',
            'kg', 'килограмм', 'weight', 'вес', 'volume', 'объем'
        ]
        
        text_lower = text.lower()
        return any(keyword in text_lower for keyword in air_keywords)

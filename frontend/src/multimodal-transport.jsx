...
function MultimodalTransportPage({ token, user }) {
  const [form, setForm] = React.useState({
    ...
    special_conditions: '',
    container_type: '20ft',
    transport_combination: 'sea_rail'
  })
  ...
  const downloadKP = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `multimodal_offer_${offerId}.pdf`
      a.click()
  ...
  const getTransportCombinationName = (combination) => {
    const combinations = {
      'sea_rail': 'Море + Железная дорога',
      'sea_auto': 'Море + Автомобиль',
      'air_auto': 'Самолёт + Автомобиль',
      'rail_auto': 'Железная дорога + Автомобиль',
      'sea_rail_auto': 'Море + Железная дорога + Автомобиль'
    }
          ...
          <div className="results-section">
            {rows ? (
              <>
                <div className="results-header">
                  ...
                  <div className="results-meta">
                    ...
                    <span>Комбинация: {getTransportCombinationName(form.transport_combination)}</span>
                  </div>
                </div>
                  ...
                  <tbody>
                    {Array.isArray(rows) ? rows.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td>🚢🚂✈️🚛 Мультимодальная доставка</td>
                ...
                <div className="notes-section">
                  <div className="note">
                    <strong>💰 Стоимость включает:</strong> Тарификатор мульти + СВХ (если выбрано) + автовывоз (если выбрано)
                  </div>
                  <div className="note">
                    <strong>⏱️ Сроки:</strong> 10-30 дней в зависимости от комбинации транспорта и маршрута
                  </div>
                  <div className="note">
                    <strong>📋 Документы:</strong> Коносамент, ЖД накладная, документы на груз, сертификаты
                  </div>
                  <div className="note">
                    <strong>🔄 Особенности:</strong> Учитываются перегрузки, таможенные процедуры, портовые сборы
                  </div>
                  <div className="note">
                    <strong>🌍 Преимущества:</strong> Оптимальное сочетание стоимости и скорости доставки
                  </div>
                </div>
              </>
            ) : (
              <div style={{ textAlign: 'center', padding: '20px', color: '#666' }}>
                {isLoading ? (
                  '🚢🚂✈️🚛 Рассчитываем стоимость мультимодальной доставки...'
                ) : (
...
export default MultimodalTransportPage

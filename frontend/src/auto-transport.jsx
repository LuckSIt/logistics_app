...
function AutoTransportPage({ token, user }) {
  ...
  const downloadKP = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `auto_offer_${offerId}.pdf`
      a.click()
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  >
                    ...
                    <option value="40hc">40 HC (по умолчанию для моря/жд/мульти)</option>
            ...
            {rows ? (
              <>
                  ...
                  <tbody>
                    {Array.isArray(rows) ? rows.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {row?.markup_percent > 0 && (
                            <span className="markup-info">
                              {row.markup_percent}% + {row.markup_fixed || 0} ₽
                            </span>
                          )}
                      ...
                      <tr>
                        ...
                        <td>
                          {rows?.markup_percent > 0 && (
                            <span className="markup-info">
                              {rows.markup_percent}% + {rows.markup_fixed || 0} ₽
                            </span>
                          )}
                          {(!rows?.markup_percent || rows.markup_percent === 0) && '-'}
                        </td>
                ...
                <div className="notes-section">
                  <div className="note">
                    <strong>ℹ️ Информация:</strong> Автомобильные перевозки включают полную стоимость доставки от двери до двери
                  </div>
                  <div className="note">
                    <strong>⏱️ Сроки:</strong> Обычно 1-7 дней в зависимости от расстояния и типа груза
                  </div>
                  <div className="note">
                    <strong>📋 Документы:</strong> Требуется ТТН, CMR, документы на груз
                  </div>
                </div>
              </>
            ) : (
              <div style={{ textAlign: 'center', padding: '20px', color: '#666' }}>
                {isLoading ? (
                  '🚛 Рассчитываем стоимость автомобильной доставки...'
                ) : (
...
export default AutoTransportPage
